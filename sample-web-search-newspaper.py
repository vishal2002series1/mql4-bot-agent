...
def fetch_article_text(url):
    try:
        ...
        article.download()
        article.parse()
        # Optionally, you can also use article.text[:2000] to limit length
        return article.text
...
query = "How to find opensearch vector db in aws which created during creation of bedrock knowledgebase. I can locate knowledebase but I want to access the same opensearch vecotor db."

search_results = google_search_with_full_content(query, GOOGLE_API_KEY, GOOGLE_CSE_ID)

combined_text = "\n\n".join(
    f"Title: {item['title']}\nURL: {item['link']}\nContent: {item['full_text'][:2000]}" for item in search_results
)
